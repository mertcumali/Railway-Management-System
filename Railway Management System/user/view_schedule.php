...
{ 
    ...
    $statement = $db->prepare("SELECT * FROM view1 WHERE train_id= ?");
...
<?php 
	$statement = $db->prepare("SELECT trains.train_name,routes.* FROM trains INNER JOIN routes ON trains.train_id=routes.train_id WHERE trains.train_id= ?");
	$statement->execute(array($train_id));
	$result = $statement->fetchAll(PDO::FETCH_ASSOC);
	foreach ($result as $row) { 
